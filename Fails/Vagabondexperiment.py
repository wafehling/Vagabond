...
class Object:
	...
	def clear(self):
		global CAMERA_W_MIN, CAMERA_W_MAX, CAMERA_H_MAX, CAMERA_H_MIN
		if self.x >= CAMERA_W_MIN:
			if self.x <= CAMERA_W_MAX:
				if self.y >= CAMERA_H_MIN:
					if self.y <= CAMERA_H_MAX:
...
def camera_move():
	global CAMERA_W_MIN, CAMERA_W_MAX, CAMERA_H_MAX, CAMERA_H_MIN
	if player.x <= SCREEN_WIDTH_MIN + 10:
		CAMERA_W_MIN = CAMERA_W_MIN - 1
		CAMERA_W_MAX = CAMERA_W_MAX - 1
	if player.x >= SCREEN_WIDTH - 10:
		CAMERA_W_MAX = CAMERA_W_MAX + 1
		CAMERA_W_MIN = CAMERA_W_MIN + 1
	if player.y <= SCREEN_HEIGHT_MIN + 10:
		CAMERA_H_MAX = CAMERA_H_MAX - 1
		CAMERA_H_MIN = CAMERA_H_MIN - 1
...
def render_all():
    ...
    if fov_recompute:
        ...
        for y in range(CAMERA_H_MIN, CAMERA_H_MAX):
            for x in range(CAMERA_W_MIN, CAMERA_W_MAX):
	...
	for object in objects:
		if object.x >= CAMERA_W_MIN:
			if object.x <= CAMERA_W_MAX:
				if object.y >= CAMERA_H_MIN:
					if object.y <= CAMERA_H_MAX:
						object.draw()
