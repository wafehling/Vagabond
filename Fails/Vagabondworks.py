...
cameratopx = 0
cameratopy = 0
camerabottomx = cameratopx + SCREEN_WIDTH
camerabottomy = cameratopy + SCREEN_HEIGHT
...
def camera_chase(dx, dy):
	global cameratopx, cameratopy, camerabottomx, camerabottomy
	if cameratopx + dx >= 0 and cameratopy + dy >= 0 and camerabottomx + dx < MAP_WIDTH-2 and camerabottomy + dy < MAP_HEIGHT-2:
		cameratopx = cameratopx + dx
		camerabottomx = camerabottomx + dx
		cameratopy = cameratopy + dy
		camerabottomy = camerabottomy + dy
	print "cameratopx: " + str(cameratopx)
	print "cameratopy: " + str(cameratopy)
	print "camerabottomx: " + str(camerabottomx)
	print "camerabottomy: " + str(camerabottomy)
...
def render_all():
    ...
    if fov_recompute:
        ...
        for y in range(cameratopy, camerabottomy):
            for x in range(cameratopx, camerabottomx):
                visible = libtcod.map_is_in_fov(fov_map, x+cameratopx, y+cameratopy)
                wall = map[x+cameratopx][y+cameratopy].block_sight
                if not visible:
                    #it's out of the player's FOV
					if map[x+cameratopx][y+cameratopy].explored:
						if wall:
                    ...
                    #it's visible
						map[x+cameratopx][y+cameratopy].explored = True
						if wall:
...
def clear_all():
	for y in range(SCREEN_HEIGHT):
		for x in range(SCREEN_WIDTH):
			lbc.console_set_back(clear, x, y, lbc.black, lbc.BKGND_SET)
...
clear = libtcod.console_new(SCREEN_WIDTH, SCREEN_HEIGHT)
lbc.sys_set_fps(LIMIT_FPS)

def player_turn():
	global player_defense
	global player_power
	player_defense = lbc.random_get_int(0, 4, 6)
	player_power = lbc.random_get_int(0, 4, 6)
player_turn()
fighter_player = Fighter(hp=55, defense=player_defense, power=player_power, death_function=player_death, attack_mode_stat = True)
